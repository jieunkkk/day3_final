...
from src.config import POLICY_LABELS, DATA_DIR
from src.data_loader import DATA_PATH, load_secom


def _check_paths() -> None:
    if not DATA_PATH.exists():
        raise FileNotFoundError(f"데이터 파일 없음: {DATA_PATH}")
    if not (ROOT / "experiments" / "registry.csv").exists():
        raise FileNotFoundError("experiments/registry.csv 없음 — 실험을 먼저 실행하세요.")
...
@st.cache_data(show_spinner="오분류 분석...")
def cached_misclassifications(exp_id: str, threshold: float):
    oof = load_oof(exp_id)
    X, y = load_secom()
...
def render_dashboard():
    ...
    with tab1:
        ...
        for stage_val in sorted(registry["stage"].unique()):
            ...
            rows.append({
                ...
                "best_pr_auc": best["pr_auc_mean"],
                "best_f1": best["f1_mean"],
            })
        stage_agg = pd.DataFrame(rows)
        st.plotly_chart(
            px.bar(stage_agg, x="stage", y="best_pr_auc", text="best_exp", title="Stage별 Best PR-AUC"),
        ...
        if not pairwise_base.empty:
            ...
            st.plotly_chart(
                px.scatter(
                    pv, x="pr_auc_experiment", y="delta", color="significant_holm",
                    hover_data=["experiment", "p_value"],
                    title="Fold PR-AUC 개선 (Δ = experiment − baseline)",
                ),
                use_container_width=True,
            )
            st.dataframe(
                pv[["experiment", "pr_auc_experiment", "delta", "p_value", "cohens_d", "significant_holm"]],
                use_container_width=True,
            )
    ...
    with tab2:
        ...
        if not st.session_state.get("advanced_loaded"):
            ...
            method = imp_df["method"].iloc[0] if "method" in imp_df.columns else "mutual_info"
            ...
            fig_imp = px.bar(
                imp_df.head(25), x="importance", y="feature", orientation="h",
                title=f"Top 25 Feature Importance ({selected_exp})",
            )
            ...
            if not cmp["detail"].empty:
                ...
                st.markdown(
                    f"**해석**: 중요 변수 유의 비율 {top_sig*100:.0f}% vs 비중요 {bot_sig*100:.0f}% — "
            ...
            if len(top_feats) >= 2:
                st.plotly_chart(
                    px.imshow(X[top_feats].corr(method="spearman"), text_auto=".2f", title="Top 10 Spearman 상관"),
                    use_container_width=True,
                )
    ...
    with tab3:
        ...
        errors = mis["errors"]
        if errors.empty:
            st.success("선택한 threshold 기준 오분류(FN/FP)가 없습니다.")
        else:
            col_a, col_b = st.columns(2)
            with col_a:
                st.markdown("**아쉬운 FN** (threshold 조정 가능)")
                fn_near = errors[errors["error_subtype"] == "FN_아쉬운(근접)"]
                st.dataframe(fn_near[["sample_idx", "y_proba", "label_audit"]].head(10) if not fn_near.empty else pd.DataFrame())
            with col_b:
                st.markdown("**확신적 FN** (모델 한계)")
                fn_hard = errors[errors["error_subtype"] == "FN_확신적오답"]
                st.dataframe(fn_hard[["sample_idx", "y_proba", "label_audit"]].head(10) if not fn_hard.empty else pd.DataFrame())

            st.subheader("케이스 직접 검토")
            subtypes = errors["error_subtype"].unique().tolist()
            filters = st.multiselect("유형", subtypes, default=subtypes[: min(2, len(subtypes))])
            pool = errors[errors["error_subtype"].isin(filters)]
            if len(pool):
                si = st.selectbox(
                    "샘플", pool["sample_idx"].tolist(),
                ...
                if imp_df is not None:
                    ...
                    prof = pd.DataFrame({
                        "sensor": imp_top,
                        "z_vs_pass": (
                            (X.iloc[si][imp_top] - X.loc[y == 0, imp_top].median())
                            / (X.loc[y == 0, imp_top].std() + 1e-9)
                        ).values,
                    })
                    st.plotly_chart(
                        px.bar(prof, x="sensor", y="z_vs_pass", color="z_vs_pass",
                               color_continuous_scale="RdBu_r", title=f"Sample #{si} Top 센서 z-score"),
                        use_container_width=True,
                    )

            st.plotly_chart(
                px.scatter(errors, x="y_proba", y="abs_dist_threshold", color="error_subtype",
                           hover_data=["sample_idx", "label_audit"]),
                use_container_width=True,
            )
...
try:
    ...
    st.markdown(
        f"- 데이터 경로: `{DATA_PATH}` (exists={DATA_PATH.exists()})\n"
        f"- 데이터 디렉터리: `{DATA_DIR}`"
    )
